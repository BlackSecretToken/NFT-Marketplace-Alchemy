...
import reducer from '../app/connectWallet';
import { useSelector, useDispatch } from 'react-redux'
import { connect } from '../app/connectWallet'



function Navbar() {
  const dispatch = useDispatch()
  const connected = useSelector((state) => state.connectWallet.connected)
  ...
  const connectWallet = async () => {
    if (window.ethereum) {
      try {
        ...
        dispatch(connect({connected: true, currAddress: addressArray[0]}));
